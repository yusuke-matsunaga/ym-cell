...
FileRegion
get_loc(const vector<AstNode*>& elem_list)
{
...
DotlibList::DotlibList(const vector<AstNode*>& elem_list) :
  AstNode(get_loc(elem_list))
{
  mNum = elem_list.size();
...
DotlibList::get_string_from_value_list() const
{
  ...
  const AstNode* value = list_elem(0);
  ShString str;
			       ...
			       ShString& str2) const
{
  ...
  const AstNode* node1 = list_elem(0);
  ...
  const AstNode* node2 = list_elem(1);
