...
HandlerFactory::new_library(DotlibParserImpl& parser)
{
  ...
  // group statements
  DotlibHandler* tmpl_handler = new_template(handler);
  DotlibHandler* group_handler = new_group(handler);
  handler->reg_handler(ATTR_CELL,   		                    new_cell(handler));
  handler->reg_handler(ATTR_DC_CURRENT_TEMPLATE,                    tmpl_handler);
  handler->reg_handler(ATTR_EM_LUT_TEMPLATE,                        tmpl_handler);
  handler->reg_handler(ATTR_FALL_TRANSITION_DEGRADATION,            group_handler);
  handler->reg_handler(ATTR_FAULTS_LUT_TEMPLATE,                    tmpl_handler);
  handler->reg_handler(ATTR_INPUT_VOLTAGE,		            new_input_voltage(handler));
  handler->reg_handler(ATTR_IV_LUT_TEMPLATE,                        tmpl_handler);
  handler->reg_handler(ATTR_LU_TABLE_TEMPLATE,                      tmpl_handler);
  handler->reg_handler(ATTR_NOISE_LUT_TEMPLATE,                     tmpl_handler);
  handler->reg_handler(ATTR_OPERATING_CONDITIONS,	            new_operating_conditions(handler));
  handler->reg_handler(ATTR_OUTPUT_CURRENT_TEMPLATE,                tmpl_handler);
  handler->reg_handler(ATTR_OUTPUT_VOLTAGE,		            new_output_voltage(handler));
  handler->reg_handler(ATTR_PART,                                   group_handler);
  handler->reg_handler(ATTR_POLY_TEMPLATE,                          tmpl_handler);
  handler->reg_handler(ATTR_POWER_LUT_TEMPLATE,                     tmpl_handler);
  handler->reg_handler(ATTR_POWER_POLY_TEMPLATE,                    tmpl_handler);
  handler->reg_handler(ATTR_POWER_SUPPLY,                           group_handler);
  handler->reg_handler(ATTR_PROPAGATION_LUT_TEMPLATE,               tmpl_handler);
  handler->reg_handler(ATTR_RISE_TRANSITION_DEGRADATION,            group_handler);
  handler->reg_handler(ATTR_SCALED_CELL,                            group_handler);
  handler->reg_handler(ATTR_SCALING_FACTORS,                        group_handler);
  handler->reg_handler(ATTR_TIMING,                                 group_handler);
  handler->reg_handler(ATTR_TIMING_RANGE,                           group_handler);
  handler->reg_handler(ATTR_TYPE,                                   group_handler);
  handler->reg_handler(ATTR_WIRE_LOAD,                              new_wire_load(handler));
  handler->reg_handler(ATTR_WIRE_LOAD_SELECTION,		    new_wire_load_selection(handler));
  handler->reg_handler(ATTR_WIRE_LOAD_TABLE,		            new_wire_load_table(handler));
...
HandlerFactory::new_pin(GroupHandler* parent)
{
  ...
  DotlibHandler* flt_simple = new FloatSimpleHandler(handler);
  DotlibHandler* func_handler = new FuncHandler(handler);
  DotlibHandler* dir_handler = new CellPinDirectionHandler(handler);
  ...
  handler->reg_handler(ATTR_DIRECTION,                           dir_handler);
  handler->reg_handler(ATTR_DONT_FAULT,                          simple);
  ...
  handler->reg_handler(ATTR_THREE_STATE,                         func_handler);
